...
'''
Description of Program: This script accepts two required input files. 1) a fasta file of ASV sequences from DADA2 pipeline, truncated to v4 region, 2) a fasta file of a genus downloaded from NCBI FL16SDB, also truncated to V4 region.
...
The ASV fasta file must contain this "size" information in the header. This threshold can be changed depending on your data.
Lastly, an identifier is added, either "dada2" or "ncbi" so read's origin can be easily known for analysis.
...
def filter_single_reads(asv_fasta_file):
    ...
    for handle in SeqIO.parse(handles, "fasta"):
        ...
        pre_filt_count+=1
    print("Number of V4 reads before filtering:", pre_filt_count)
    ...
    for i in filtered_reads:
        post_filt_count+=1
    print("Number of V4 reads after filtering:", post_filt_count)
...
def get_pacbio_seqs(filtered_reads):
    ...
    with open("filtered_records_v4.fasta", "w") as fr:
        for handle in SeqIO.parse(handles, "fasta"):
    ...
    return "filtered_records_v4.fasta"
...
def combine_asv_rep_w_ncbi(new_records, ncbi_records):
    with open(ncbi_records, "r") as ncbi_file, open("ncbi_seqs_no_blanks_v4.fasta", "w") as ncbi_out_file, open(new_records, "r"):
    #first remove blank lines from ncbi fastas
        for line in ncbi_file:
            if line.strip():
                ncbi_out_file.write(line)

        files_to_combine = ["ncbi_seqs_no_blanks_v4.fasta", new_records]
        #now combine the files in a new file containing all of the filtered records
    with open("all_filtered_seqs_v4.fasta", "w") as all_record_file:
        ...
        return "all_filtered_seqs_v4.fasta"
...
def print_total_num_of_seqs(all_filtered_records_file):
    with open(all_filtered_records_file, "r") as f:
        ...
        print("Total number of records in all_filtered_seqs_v4:", total_seqs)
...
def make_all_reads_info_tsv(all_filtered_reads_w_id):
    ...
    with open(all_filtered_reads_w_id, "r") as f:
        ...
        with open("all_reads_info_v4.tsv",  'w') as ari_tsv:
            #ncbi
...
def get_all_seqs(all_filtered_seqs_file, post_all_seqs_list):
    ...
    i = 0
    total_records = 0
    with open("all_filtered_seqs_w_id_v4.fasta", "w") as fr:
        for line in handles:
    ...
    handles.close()
    print("Number of records wrote to all_filtered_seqs_w_id_v4.fasta", total_records)

    return "all_filtered_seqs_w_id_v4.fasta"
...
if __name__ == '__main__':
    #Get Input
    asv_fasta_file = glob.glob("*_asv_v4.fasta")
    ncbi_records = glob.glob("*_ncbi_v4.fasta") 
    ...
    print("\nAll pacbio and ncbi reads have been written to all_filtered_seqs_v4.fasta")
    ...
    print("\nAll reads info wrote to all_reads_info_v4.tsv")
    print("Fin")
